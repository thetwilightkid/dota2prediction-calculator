...
export const patchImpactMeta = {
  "patch": "7.41e",
  "method": "patch_impact_score is a live, data-driven signal - NOT derived from patch-notes text. For each hero, OpenDota's heroStats exposes pub_pick_trend/pub_win_trend, a rolling 7-bucket window of recent pub match pick/win counts. We split it early (buckets 0-2) vs late (buckets 4-6, bucket 3 dropped as a buffer) and take the win-rate difference (late - early) as that hero's winrate_delta - a genuine 'is this hero trending up or down right now' number, not a guess from reading patch notes. Since patch 7.41e is only about a week old at collection time, this entire 7-bucket window falls within the new patch, so the early/late split captures how the pub meta is still adjusting to it, rather than a clean old-patch-vs-new-patch comparison (no reliable pre-patch baseline was available: Stratz's own per-patch winGameVersion breakdown is stale and doesn't have entries for 7.41e yet). Each team's patch_impact_score is the pick-centrality-weighted average winrate_delta across their top 8 most-picked heroes (league_data_mode from team_pick_ban_stats.json) - positive means their signature pool is trending toward higher win rates post-patch, negative means the opposite. This is intentionally a small-magnitude, secondary signal, not a primary rating input - it is NOT folded into compute_ratings.py's composite rating.",
  "min_bucket_picks_for_reliable_winrate": 200,
  "top_n_pool_heroes": 8,
...
export const heroTrends: Record<string, HeroTrend> = {
  "1": {
    "hero_name": "Anti-Mage",
    "pub_pick_trend": [
      74852,
